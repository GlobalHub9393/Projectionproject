...
async function updateField(sql, table, field, value, whereSql, args=[]) {
  // table and field are validated against hard-coded allowlists before arriving here.
...
export default async function handler(req,res) {
  try {
    ...
    // No setup screen anymore. First request creates/seeds Neon automatically.
    await ensureDatabase(sql);

    if (req.method === 'GET') {
      return res.status(200).json(await snapshot(sql));
    }
    if (req.method !== 'POST') {
    ...
    if (b.action === 'save_config') {
      ...
      await sql`INSERT INTO coaching_points(month,scope,rep_id,kind,title,body,sort_order)
        VALUES((SELECT active_month FROM app_config WHERE id=1),
        ${b.rep_slug?'rep':'store'},${repId},${b.kind||'coach'},
        ${b.title||''},${b.body||''},${Number(b.sort_order||100)})`;
      ...
      await sql`INSERT INTO sbs_observations(
        rep_id,observed_on,title,greet,understand,recommend,confirm,educate,thank,notes,coaching_summary)
