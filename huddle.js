...
async function snapshot(sql) {
  ...
  const reps = await sql`
    SELECT r.*,
      ...
      COALESCE((to_jsonb(rg) - 'month' - 'rep_id' - 'updated_at'),'{}'::jsonb) AS goal_overrides
    FROM reps r
    LEFT JOIN rep_metrics m ON m.rep_id=r.id AND m.month=${month}
    LEFT JOIN rep_goals rg ON rg.rep_id=r.id AND rg.month=${month}
    WHERE r.active=true
    ORDER BY r.sort_order,r.display_name`;
  const coaching = await sql`SELECT c.*,r.slug AS rep_slug,r.display_name AS rep_name
    FROM coaching_points c
    LEFT JOIN reps r ON r.id=c.rep_id
    WHERE c.month=${month} AND c.active=true
    ORDER BY c.sort_order,c.updated_at DESC`;
  const sbs = await sql`SELECT s.*,r.slug AS rep_slug,r.display_name AS rep_name
...
async function updateField(sql, table, field, value, whereSql, args=[]) {
  const query = `UPDATE ${table} SET ${field}=$1, updated_at=now() WHERE ${whereSql}`;
...
export default async function handler(req,res) {
  try {
    ...
    if (b.action === 'verify_admin') return res.status(200).json({ok:true});

    if (b.action === 'save_config') {
      await sql`UPDATE app_config SET
        headline=${b.headline||''},
      ...
      for (const [k,v] of Object.entries(b.fields||{})) {
        if (goalFields.has(k)) {
          await updateField(sql,'monthly_goals',k,v,'month=(SELECT active_month FROM app_config WHERE id=1)');
        }
      }
      ...
      await sql`INSERT INTO rep_goals(month,rep_id)
        VALUES((SELECT active_month FROM app_config WHERE id=1),${rep.id})
        ON CONFLICT(month,rep_id) DO NOTHING`;
      for (const [k,v] of Object.entries(b.fields||{})) {
        if (repGoalFields.has(k)) {
          await updateField(sql,'rep_goals',k,(v===null||v===''?null:v),
      ...
      const rep = (await sql`SELECT id FROM reps WHERE slug=${b.rep_slug}`)[0];
      if (!rep) throw new Error('Rep not found');
      await sql`DELETE FROM rep_goals
        WHERE rep_id=${rep.id} AND month=(SELECT active_month FROM app_config WHERE id=1)`;
      ...
      for (const [k,v] of Object.entries(b.fields||{})) {
        if (allowedRepFields.has(k)) {
          await updateField(sql,'rep_metrics',k,v,
            'rep_id=$2 AND month=(SELECT active_month FROM app_config WHERE id=1)',[rep.id]);
        }
      }
      ...
      await sql`INSERT INTO coaching_points(month,scope,rep_id,kind,title,body,sort_order,status)
        VALUES((SELECT active_month FROM app_config WHERE id=1),
        ${b.rep_slug?'rep':'store'},${repId},${b.kind||'coach'},
        ${b.title||''},${b.body||''},${Number(b.sort_order||100)},${b.status||'new'})`;

    } else if (b.action === 'set_coaching_status') {
      await sql`UPDATE coaching_points SET status=${b.status||'new'},updated_at=now() WHERE id=${b.id}`;
      ...
      await sql`INSERT INTO sbs_observations(
        ...
        VALUES(${rep.id},${b.observed_on||new Date().toISOString().slice(0,10)},
        ${b.title||''},${b.greet||null},${b.understand||null},${b.recommend||null},
        ${b.confirm||null},${b.educate||null},${b.thank||null},
        ${b.notes||''},${b.coaching_summary||''},${JSON.stringify(b.behavior_notes||{})}::jsonb)`;
