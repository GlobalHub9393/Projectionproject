...
export async function ensureDatabase(sql) {
  ...
  // Placeholder September goals — intentionally editable in Admin.
  ...
  if (coaching[0].n === 0) {
    await sql`INSERT INTO coaching_points(month,scope,kind,title,body,sort_order) VALUES
      ('2026-09-01','store','shoutout','Fresh Start',
       'September starts at zero. Build the month one complete interaction at a time.',1),
      ('2026-09-01','store','coach','Complete Recommendation',
       'Do not just accept no. Ask why, note the pushback, and adjust the recommendation.',2),
      ('2026-09-01','store','finish','Finish the Whole Sale',
       'Rerate the account, look for more than one upgrade, present protection, accessories, Home Internet, and trade when they fit.',3)`;
  }
}
