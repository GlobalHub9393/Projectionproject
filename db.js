...
export function requireAdmin(req) {
  const expected =
    process.env.ADMIN_PHRASE || 'Hello?';

  const supplied =
    req.headers['x-admin-phrase'];
...
export async function ensureDatabase(sql) {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS app_config (
      ...
      opening_prompt text NOT NULL
        DEFAULT 'September starts at zero. What are you going after today?',
      team_message text NOT NULL
        DEFAULT 'New month. Clean board. Every interaction matters.',
  ...
  await sql`
    INSERT INTO store_metrics(month)
    VALUES('2026-09-01')
    ON CONFLICT(month) DO NOTHING
  `;
    ...
    await sql`
      ...
      VALUES
        (
          ...
          'shoutout',
          'Fresh Start',
          'September starts at zero. Build the month one complete interaction at a time.',
          1,
          'new'
        ),
        (
          ...
          'Do not just accept no. Ask why, note the pushback, and adjust the recommendation.',
          2,
          'working'
        ),
        (
          ...
          'store',
          'finish',
          'Finish the Whole Sale',
          'Rerate the account, look for more than one upgrade, present protection, accessories, Home Internet, and trade when they fit.',
          3,
          'working'
        )
    `;
  ...
  if (noelle) {
    ...
    const seed = async (
      ...
      const exists = await sql`
        SELECT 1
        FROM sbs_observations
        WHERE rep_id=${noelle.id}
      ...
      if (!exists.length) {
        ...
        await sql`
          ...
          VALUES(
            ...
            ${notes},
            ${summary},
            ${JSON.stringify(behaviorNotes)}::jsonb
          )
        `;
      }
    };
    ...
    await seed(
      ...
      {
        greet:
          'Let customers come to her, but did greet within the expected window.',
        understand:
          'Asked Live, Work, Play questions and identified the need for port information and a new phone.',
        recommend:
          'Recommended Internet, plan benefits, insurance and Next Up; accessories needed to be physically brought into the recommendation.',
        confirm:
          'Confirmed plan fit and that the reason for the visit was resolved.',
        educate:
          'Explained a lot and offered content-transfer help; first-bill expectations needed more attention.',
        thank:
          'Appreciative close, but contact information was not shared.'
      }
    );
    ...
    await seed(
      ...
      {
        greet:
          'Stayed seated during the greeting and initially directed the customers toward customer care before moving into the account.',
        understand:
          'Used Live, Work, Play questions and found device and Internet opportunities.',
        recommend:
          'Used the sales tools and did a strong job offering the Internet promotion.',
        confirm:
          'Moved through the explanation very quickly; the customer may not have fully absorbed the value.',
        educate:
          'Explained next steps and even offered help setting up watches; Internet pickup was planned for later.',
        thank:
          'Walked the customers out, but did not share contact information.'
      }
    );
    ...
    await seed(
      ...
      {
        greet:
          'Waited for the customer to approach instead of actively welcoming; translator was needed.',
        understand:
          'Spoke carefully through the translator and surfaced the customer’s concerns.',
        recommend:
          'Customer knew what he wanted; accessories, AIA, and data-line benefits were not presented.',
        confirm:
          'Reviewed what was being added, including PA4 and future upgrade options; network reinforcement was limited.',
        educate:
          'Customer did not fully understand through the translator and was ready to leave.',
        thank:
          'Reviewed future upgrade options and used the QR code so the customer could reconnect later.'
      }
    );
    ...
    await seed(
      ...
      {
        greet:
          'Greeted the customer while seated.',
        understand:
          'Asked about the desired phone, color, and trade-in but did not broaden discovery.',
        recommend:
          'Only Next Up Anytime was presented; no accessory, Internet, or connected-device recommendation.',
        confirm:
          'The customer’s requested upgrade was handled, but the full value of the solution was not reinforced.',
        educate:
          'No meaningful current-bill or future-bill conversation during the sales process.',
        thank:
          'Basic close; customer was told the store would reach out when the phone arrived.'
      }
    );
    ...
    await seed(
      ...
      {
        greet:
          'Seated during the greeting, though very pleasant during introductions.',
        understand:
          'Reassured the customers and showed empathy, but did not ask about the business account or use Live, Work, Play questions.',
        recommend:
          'Rerate/data-device opportunity and the Buy 2 Get 1 accessory conversation were missed.',
        confirm:
          'Effectively confirmed that this was all the customers wanted and offered future support.',
        educate:
          'Content transfer and next steps were explained, but trade value was not explained clearly.',
        thank:
          'Warm goodbye, but the walk-out, tenure thank-you, and contact information were missed.'
      }
    );
    ...
    await seed(
      ...
      {
        greet:
          'Friendly, but remained seated and the greeting was not very personal.',
        understand:
          'Accessed the account and reviewed opportunities, though the behavior was rated Needs Improvement.',
        recommend:
          'Three upgrades were declined; Noelle kept working and found Open Enrollment as a relevant alternative.',
        confirm:
          'On Track.',
        educate:
          'On Track.',
        thank:
          'Did not share contact information and remained seated while the customer left.'
      }
    );
